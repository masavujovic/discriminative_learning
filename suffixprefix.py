...
class SuffixPrefixModel:
    def __init__(self, exemplars, probabilities, categories, number_of_categories, condition, alpha, beta):
    ...
    def rescorla_wagner(self):
        ...
        if self.condition == 'suffix':
            ...
            if type(self.alpha) != float:
                self.theta[label,:] += self.alpha[label,:] * self.beta * (outcome - v_total) * self.exemplars[choice,:]
            else:
                self.theta[label,:] += self.alpha * self.beta * (outcome - v_total) * self.exemplars[choice,:]
        else:
            ...
            if type(self.alpha) != float:
                self.theta[cat_of_choice,:] += self.alpha[cat_of_choice,:] * self.beta * (outcome - v_total)
            else:
                self.theta[cat_of_choice,:] += self.alpha * self.beta * (outcome - v_total)
