...
def averaged_weights(weights_init, learning_set, probabilities, categories, number_of_categories, condition, alpha, beta, n_trials):
    ...
    while i < 10:
        if condition == "prefix":
            network = Simulation(learning_set, probabilities, categories, number_of_categories, "prefix", alpha, beta, n_trials)
            new_weights = network.simulate()
        else:
            network = Simulation(learning_set, probabilities, categories, number_of_categories, "suffix", alpha, beta, n_trials)
            new_weights = network.simulate()
